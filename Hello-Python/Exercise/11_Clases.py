...
class BankAccount:
  ...
  def whithdrow_money(self, money):
    if (money <= self.balance):
      self.balance -= money
    else: print(f"No hay suficiente dinero. Sus fondos actuales son {self.balance}")
...
class Store:
  def __init__(self, invetory = None):
    self.inventory = invetory if invetory is not None else [] #Si se da una lista, self.inventory sera esa lista, sino, se crea una lista nueva vacia
