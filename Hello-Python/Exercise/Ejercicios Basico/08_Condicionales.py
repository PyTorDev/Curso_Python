...
def mayor_edad():  
  ...
  else: print(f"Vaya, con {age} todavia eres menor de edad")
...
def cadena_vacia():  
  ...
  else: print("Es una cadena de texto, pero esta vacia")
...
def divisible():
  ...
  else: print(f"{n} no es divisible entre 3 y 5 a la vez")
...
def votar():  
  ...
  if (age >= 18):
    ...
    print(f"Buno, con {age} años, puedes vota pero necesitas un permiso especial de tu tutor legal")
  else: print(f"Vaya, con {age} todavia no puedes votar")
...
def passw():
  ...
  else: print("Error. La contraseña no coincide")
...
def in_range(min = 10, max = 20):
  ...
  else: print(f"{n} no esta entre {min} y {max}")
...
def semaforo():
  ...
  else: print(f"{color} no es un color correcto")
